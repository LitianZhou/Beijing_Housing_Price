...
selection <-regsubsets(mat_full,exampledata$data$price,int = F,really.big = T,method = "exhaust", nvmax = 11)
coef(selection,11)
...
exampledata$data$season <- as.factor(exampledata$data$season)
model_select <- lm(price ~ square + bathroom + buildingtype + constructiontime + renovationcondition + elevator + subway + district +season, data = exampledata$data)
...
reference <- model_select$coefficients[1] + mean(exampledata$data$bathroom)*model_select$coefficients[3] + sum(weight_dist*c(0,model_select$coefficients[12:22])) 
...
plot(residual) ## residual plot between consecutive season
```

As we see price differences between adjacant seasons, we found strong correlation between these residuals and they tend to become unstable as the price went up. So we decided to calculate the log of the rate of the price change and observe its residual. 
```{r}
...
```
The residual plot looks better as the log rate change is more close to 0 and tend to become stable across the time. Therefore, we assume the log rate difference between adjacent seasons tend to be a constant around zero. For time series model ARIMA$(p,d,q)$, this assumption indicates d = 1.
...
<<<<<<< HEAD
The perdicted house price in 2018 to 2019 from the fitted ARIMA(8,1,1) is shown in the following price trend graph:  
```{r}
=======

```{r, fig.height = 3, fig.width = 5}
>>>>>>> 6797b64030164c1bf51ce635d0ccb87674669c05
