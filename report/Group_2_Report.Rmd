...
exampledata = gen_full_data("HaiDian", c("Tower", "Plate"))
```

# Statistical Model
As we first to descide a model to use, we tried the full model first, that fit all the covariates above excluding the total price in the linear regression model and the 'price' as the output.
...
```
As we see from the result of model selection from example data we choose covarites 'square','floor','renovationcondition','elevator','subway','season' as our covarites for best mallow's Cp, which achieve a balance in baisness and estimate variance.
We actually choose simliar model for the full data set with covariates 'square','bathroom','buildingtype','renovationcondition', 'district', 'elevator', 'subway' and factorized 'season'.
