...
Filter_model <- function(inquaried_data){
  ...
  plotxaxis <-as.numeric(names(table(subdata$season)))
  subdata$season <- as.factor(subdata$season)
  if (inquaried_data[[4]] == F){
  ...
  }else if(inquaried_data[[2]]==T && inquaried_data[[3]]==F){
  ...
  }else if(inquaried_data[[2]]==F && inquaried_data[[3]]==T){
  ...
  beta_data <- data.frame(matrix(c(plotxaxis,(plotxaxis[length(plotxaxis)]+1):(plotxaxis[length(plotxaxis)]+10),lower_beta,betas),(10+num_season),3))
  beta_data <- gather(beta_data,"class","price",2:3)
  names(beta_data)[1] <- "season"
  ggplot(beta_data) + aes(x = season, y = price , color = class, stroke = 2) + geom_line(size=1.5)
}