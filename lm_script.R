...
##Function Conneced to Database inquiry data
Filter_model <- function(inquaried_data){
  library(forecast)
  subdata <- inquaried_data[[1]]
  subdata$season <- as.factor(as.numeric(subdata$season))
  ...
  lower_beta <- lower_beta[-length(lower_beta)]
  ##higher_beta <- higher_beta[-length(higher_beta)]
  beta_data <- data.frame(matrix(c(1:(10+num_season),lower_beta,betas),(10+num_season),3))
...
beta_data <- gather(beta_data,"class","price",2:4)
ggplot(beta_data) + aes(x = X1, y = price , color = class, stroke = 2) + geom_line(size=1)
